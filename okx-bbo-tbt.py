...
async def okx_get(symbol_spot, symbol_swap):
    ...
    try:
        ...
        # print(fee_swap)
        bbo = await exchange.public_get_market_ticker({'instType': 'SWAP', 'instId': symbol_swap + '-SWAP'})
        swap = bbo['data'][0]
        # print(swap)
        ...
        point = (
            ...
            .field("bidVolume", ticker['bidVolume']))  # 写入数据点到InfluxDB
...
async def huobi_get(symbol_spot, symbol_swap):
    ...
    try:
        ...
        fee = await exchange.fetch_trading_fee(symbol_spot)
        fee_swap = await exchange.contract_public_get_linear_swap_api_v1_swap_funding_rate(
            {'contract_code': symbol_swap})
        # print(fee_swap)
        bbo = await exchange.contract_public_get_linear_swap_ex_market_bbo({'contract_code': symbol_swap})
        # print(bbo)
        swap = bbo['ticks'][0]
    ...
    while(True):
        for i in range(len(symbols)):
            ...
            if (ticker_dict_spot and ticker_dict_swap):
                ...
                cal_price_difference(ticker_dict_spot.get('okx'), ticker_dict_spot.get('huobi'))
                cal_price_difference(ticker_dict_swap.get('okx'), ticker_dict_swap.get('huobi'))
        time.sleep(60)
    # coinbase、币安、mex
    # 加狗狗币和eth
